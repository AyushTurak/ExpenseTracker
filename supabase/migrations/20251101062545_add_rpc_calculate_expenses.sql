/*
  ...
  1. Functions
    ...
    - `check_budget_thresholds` - Check if user exceeded budget thresholds
...
CREATE OR REPLACE FUNCTION calculate_budget_percentage(user_id_param uuid)
RETURNS numeric AS $$
DECLARE
  budget numeric;
  expenses numeric;
...
BEGIN
  ...
  expenses := calculate_user_expenses(user_id_param);
...
BEGIN
  ...
  percentage := calculate_budget_percentage(user_id_param);
